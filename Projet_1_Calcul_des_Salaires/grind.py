...
def monthly_salary(): 
    ...
    puis calcule et affiche les statistiques des salaires globales et par filiale.
    """
    ...
    # Afficher les employés par filiale sous forme de tableau et calculer les statistiques pour chaque filiale
    for filliale, employees in filliales.items():
        ...
        # Calcul des statistiques par filiale
        mean_salary, max_salary, min_salary = calculate_statistics(employees)

        print(f"\nStatistiques des salaires pour la filiale {filliale} :")
        print(f"Salaire moyen: {mean_salary:.2f}")
        print(f"Salaire maximum: {max_salary:.2f}")
        print(f"Salaire minimum: {min_salary:.2f}")
        print("-" * 60)

    # Calculer et afficher les statistiques des salaires globales
    mean_salary_global, max_salary_global, min_salary_global = calculate_statistics(employees_salaries)

    print("\nStatistiques des salaires globales :")
    print(f"Salaire moyen: {mean_salary_global:.2f}")
    print(f"Salaire maximum: {max_salary_global:.2f}")
    print(f"Salaire minimum: {min_salary_global:.2f}")


def calculate_statistics(employees):
    """
    Calcule les statistiques des salaires pour un groupe d'employés.
    ...
    mean_salary = sum(salaries) / len(salaries) if salaries else 0
